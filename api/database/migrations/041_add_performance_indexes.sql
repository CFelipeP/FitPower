...
CREATE INDEX IF NOT EXISTS idx_sessions_trainer_date_status ON sessions(trainer_id, date, status);
...
CREATE INDEX IF NOT EXISTS idx_user_programs_program_status ON user_programs(program_id, status);

-- Payments: fast analytics queries
CREATE INDEX IF NOT EXISTS idx_payments_status_type_created ON payments(status, type, created_at);

-- Support tickets: fast open ticket queries
CREATE INDEX IF NOT EXISTS idx_tickets_severity_created ON support_tickets(severity, created_at DESC);
